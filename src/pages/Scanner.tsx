...
export default function Scanner() {
      ...
      mediaRecorder.onstop = async () => {
        ...
        if (autoDownload) {
          try {
            ...
            a.href = url;
            a.download = `Packing_${recordingFor}.webm`;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            URL.revokeObjectURL(url);
            console.log("Local PC Auto-download success for:", recordingFor);
          } catch (de) {
            console.error("Auto-download failed:", de);
          }
        }
  ...
  const uploadVideoBlob = async (blobToUpload: Blob, resiCode: string) => {
    ...
    try {
      ...
      console.error(`Upload failed for ${resiCode}:`, err);
      setError(err.message || `Gagal mengunggah video untuk resi ${resiCode}`);
    }
  };
